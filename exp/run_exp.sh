...
exp_input_path=$base_path/exp/prune_input/$server_name
task_path=$base_path/task/${server_name}
prune_path=$base_path/$prune_name
train_json_path=$base_path/task/${server_name}/exp_train_base.json
kp_question_recall_result_directory=$base_path/result/${server_name}
...
do
  ...
  # 重置图
  ../bin/$env/GraphComputeClient --server_name=${server_name} --action="RESET"
  ...
  ../bin/$env/GraphComputeClient --server_name=${server_name} --json=$task_path/exp_graph_prune.json
  echo "prune success"
  # 训练超参
  ...
  do
    ...
    do
      ...
      ../bin/GraphComputeClient --server_name=${server_name} --json=${train_json_path}
      if [[ $use_restartRatios -eq 1 ]]; then
