...
namespace arch {
    ...
    struct In {
        ...
        std::map<string, int32_t> questions_assement;
        std::map<string, double> current_knowledge_points;
        std::map<string, int32_t> preceding_questions_assement;
        std::map<string, double> knowledge_points;
    };

    struct Out {
        int32_t code ;
        std::vector<string> payload; // recommended question ids
    };
